...
public class NoDimensionOnNonVectorTargetTypesValidator implements SpecificationValidator {

    private static final String ERROR_CODE = "TYPE-002";

    private final Map<String, String> invalidDimensionPropertyNames;

    public NoDimensionOnNonVectorTargetTypesValidator() {
        this.invalidDimensionPropertyNames = new LinkedHashMap<>();
    }
    ...
    private void visitEntity(int index, String group, EntityTarget target) {
        ...
        for (int i = 0; i < mappings.size(); i++) {
            ...
            if (type != null && type.getDimension() != null && !type.getName().isVector()) {
                var path = String.format(
                        "$.targets.%s[%d].properties[%d].target_property_type.dimension", group, index, i);
                invalidDimensionPropertyNames.put(path, type.getName().name());
            }
        }
    }
}
