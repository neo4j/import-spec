...
public class TextSourceProvider implements SourceProvider<TextSource> {
    ...
    @Override
    public TextSource apply(ObjectNode node) {
        ...
        if (node.has("urls")) {
            ...
            return new ExternalTextSource(
                    ...
                    header,
                    format,
                    node.has("column_delimiter") ? node.get("column_delimiter").textValue() : DEFAULT_COLUMN_DELIMITER,
                    node.has("line_separator") ? node.get("line_separator").textValue() : DEFAULT_LINE_SEPARATOR);
        }
